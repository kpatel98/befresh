...
<body >
  ...
  <div class="form">
    <form name="l_valid" class="register-form" method="post" action="" onsubmit="return validateFormlogin()">
	<h3>Enter your detail</h3><br>
      <input type="text" placeholder="username" name="l_use" /><font color=red><p id="sl_use" ></p></font>
...
<script>
	...
	<h3>Log in</h3><br>
      <input type="text" placeholder="phone_number" name="lo_pho"/><font color=red><p id="slo_pho"></p></font>
      <input type="password" placeholder="password" name="lo_pas"/><font color=red><p id="slo_pas"></p></font>
      <button type="submit" class="btn btn-success" name="lo_log">login</button>
      <p class="message">Not registered? <a href="#">Create an account</a><br><a href=""><u>forgot password</u></a></p>
    </form>
  </div>
</div>
		...
		if (xlo_pho == "")
			{
				...
				document.getElementById("slo_pho").innerHTML = "enter valid phone number1";
...
if(isset($_POST['lo_log']))
	...
	$query="SELECT * FROM login WHERE log_phonenumber='$vlo_pho' and log_login=0";
		$result=mysqli_query($conn,$query);
		if(mysqli_num_rows($result)>0)
		{
			$query="SELECT * FROM login WHERE 	log_password='$vlo_pas' and log_phonenumber='$vlo_pho'";
			$resultt=mysqli_query($conn,$query);
			$rowid = mysqli_fetch_assoc($result);
			if(mysqli_num_rows($resultt)>0)
			{
				$_SESSION['aa']=$vlo_pho;
				$_SESSION['u_id']=$rowid['log_userid'];
			...
			elseif($vlo_pas=="")
			{	
				?>  <script>document.getElementById("slo_pas").innerHTML = " ";</script>";<?php
			}
		...
		{
			?>  <script>document.getElementById("slo_pho").innerHTML = "phone number is not register sign up";</script>";<?php
